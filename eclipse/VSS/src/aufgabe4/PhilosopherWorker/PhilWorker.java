...
public class PhilWorker implements PhilWorkerRMIInterface {
	...
	/**
	 * A remote exception is usually an indication that a worker is not reachable anymore. tell the master to take further action.
	 * @param e
	 * @param obj
	 */
	public void handleRemoteException(final RemoteException e, final AbstractRemoteObjectIF obj) {
		try {
			//just pass -1 since we may not know the object ID at this place.
			this.getMasterStub().objectWithIDIsNotReachable(-1);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	}
	...
	public void start() throws RemoteException {
		...
		if (localPhils.isEmpty()) {
			...
			try {
				this.philController = new PhilController(localPhils, this.getMasterStub().getTable());
			} catch (NotBoundException e) {
				System.err.println("Seems like the master is not reachable.");
				e.printStackTrace();
			}
		}
	}
	...
	public MasterRMIInterface getMasterStub() throws RemoteException, NotBoundException {
		MasterRMIInterface master;
		Registry registry = LocateRegistry.getRegistry(PhilWorker.masterAddress);
		master = (MasterRMIInterface) registry.lookup("master");
		return master;
